...
const compiledDePaydual = require("./build/DePaydual.json");
...
const deploy = async () => {
  ...
  const result = await new web3.eth.Contract(compiledDePay.abi)
    ...
    const result2 = await new web3.eth.Contract(compiledDePaydual.abi)
    .deploy({ data: compiledDePaydual.evm.bytecode.object })
    .send({ gas: "4000000", from: accounts[0] });
  console.log("Contract DePay deployed to", result.options.address);
  console.log("Contract DePaydual deployed to", result2.options.address);
